...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${API_BASE}/api/admin/homepage/awards`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
    body: form,
  });
  if (!response.ok) {
    const detail = await responseError(response, "荣誉图片上传失败");
  ...
  redirect(feedbackPath(adminPath, "ok", "荣誉图片已上传"));
}
