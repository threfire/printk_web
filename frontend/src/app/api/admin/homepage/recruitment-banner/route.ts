...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${API_BASE}/api/admin/homepage/recruitment-banner`, {
    method: "PUT",
  ...
  if (!response.ok) {
    const detail = await responseError(response, "招新公告栏保存失败");
  ...
  redirect(feedbackPath(adminPath, "ok", "招新公告栏已保存"));
}
