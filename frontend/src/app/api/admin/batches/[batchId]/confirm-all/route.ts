...
export async function POST(
  ...
  if (!confirmResponse.ok) {
    redirect(feedbackPath("/admin", "error", await responseError(confirmResponse, "整批确认失败")));
  }
  ...
  if (!completeResponse.ok) {
    redirect(feedbackPath(`/admin/batches/${encodeURIComponent(batchId)}`, "error", await responseError(completeResponse, "批次完成失败")));
  }
  redirect(feedbackPath("/admin", "ok", `批次 ${batchId} 已整批确认`));
}
