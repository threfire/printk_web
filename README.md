...
涉及首页弹幕时，必须保留 `homepage_danmaku` 的弹幕主键、图片地址、账号、昵称、内容、轨道、颜色、创建时间和播放参数。代码更新只能新增兼容字段或调整读取方式，不能用图片地址变更、轮播图替换或数量裁剪删除已有弹幕记录。

首页弹幕显示必须按 `homepage_danmaku.image_src` 隔离，单张图片只读取和展示该图片地址对应的弹幕记录。
